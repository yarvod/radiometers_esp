...
bool ParseConfigText(const std::string& text, AppConfig* config) {
  ...
  bool ssid_set = false, pass_set = false, usb_set = false, usb_value = false;
  ...
  while (line_start <= text.size()) {
    ...
    if (key == "wifi_ssid") {
      ...
      if (ParseBool(value, &usb_value)) usb_set = true;
      else ESP_LOGW(kTag, "Invalid usb_mass_storage value in config.txt");
    } else if (key == "pid_kp") {
  ...
  if (usb_set) { config->usb_mass_storage = usb_value; config->usb_mass_storage_from_file = true; }
  ...
  return config->wifi_from_file || config->usb_mass_storage_from_file || log_active_set ||
