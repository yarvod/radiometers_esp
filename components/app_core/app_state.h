...
struct AppConfig {
  ...
  int meteo_file_interval_s;  // CSV write interval; default 60 (independent from poll)
};
...
struct SharedState {
  ...
  float pid_output;
  float pid_temperature;
  float pid_error;
  float pid_integral;
  float pid_integral_candidate;
  ...
  float pid_dt;
  bool pid_saturated_high;
  bool pid_saturated_low;
  bool pid_integral_held;
